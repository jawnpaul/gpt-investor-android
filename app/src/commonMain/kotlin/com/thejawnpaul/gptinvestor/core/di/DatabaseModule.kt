...
import com.thejawnpaul.gptinvestor.core.database.getTopPickDao
...
val databaseModule: List<Module> = module {

    single<GPTInvestorDatabase> { getRoomDatabase(get()) }
    single<CompanyDao> { getCompanyDao(get()) }
    single<ConversationDao> { getConversationDao(get()) }
    single<MessageDao> { getMessageDao(get()) }
    single<TopPickDao> { getTopPickDao(get()) }
