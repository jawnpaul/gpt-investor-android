...
class GetSectorCompaniesUseCase @Inject constructor(@IoDispatcher private val dispatcher: CoroutineDispatcher, coroutineScope: CoroutineScope, private val repository: ICompanyRepository) :
    BaseUseCase<String?, List<Company>>(coroutineScope, dispatcher) {
    override suspend fun run(params: String?): Flow<Either<Failure, List<Company>>> = repository.getCompaniesInSector(params)
}
